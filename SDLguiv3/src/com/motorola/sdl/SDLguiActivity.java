//-----------------------------------------------------------
// Android SDL Sample Application
//
// Copyright (c) 2012-2013 Motorola Solutions, Inc.
//-----------------------------------------------------------
		...
		SurfaceHolder.Callback, BarCodeReader.VideoCallback
{
	...
	private boolean snapPreview 	= false;		// snapshot preview mode enabled
	...
	protected void onResume()
	{
		...
		try
		{
			...
			bcr = BarCodeReader.open(1); // Set this to the camera ID on the platform
			if (bcr == null)
			{
				dspErr("open failed");
				return;
...
// === Android UI methods =======================================
	...
	public void surfaceCreated(SurfaceHolder holder)
	{
		try
		{
   		if (state == STATE_PREVIEW)
   		{
				...
				bcr.startViewFinder();			//snapshot with preview mode
   		}
   		else //must be video	
   		{
				//bcr.setPreviewDisplay(holder);
	...
	OnClickListener mEnableAllListener = new OnClickListener()
	{
		public void onClick(View v)
		{
			...
			/*String FilePath = "/mnt/sdcard/CAABVS00-001-R01D0.DAT";
...
// ==== SDL methods =====================
	...
	private void doDefaultParams()
   {
	...
	{
		if (state != STATE_SNAPSHOT || image == null)
			return;			
	...
	private int setIdle()
	{
		...
		case STATE_PREVIEW:
			bcr.stopPreview();		//stop preview
			bcr.takePicture(null);	//end snapshot mode 
			break;
